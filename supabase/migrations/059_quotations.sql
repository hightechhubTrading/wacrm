...
-- SECURITY DEFINER bypasses RLS on quotation_sequences by design (see
-- note above) -- but that means THIS function is the only gate keeping
-- one account from minting/observing another account's reference
-- sequence. Without the membership check below, any authenticated user
-- of ANY account could call next_quotation_reference('<other-account>',
-- 'PIV') directly and advance/read a sequence it has no business
-- touching. Mirrors the same-shaped check in 018's set_member_role.
...
-- Derived from the PARENT quotation's account, not the item's own
-- account_id column -- that column isn't tied to the item's actual
-- parent quotation and is spoofable at insert time (nothing enforces
-- it matches quotations.account_id for quotation_id). Matches the
-- quotation_items_write pattern below.
