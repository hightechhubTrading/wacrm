...
import { HANDOFF_SENTINEL, MEDIA_SENTINEL_OPEN, MEDIA_SENTINEL_CLOSE, aiRequestTimeoutMs } from './defaults'
...
/**
 * Split the raw model output into `{ text, handoff, mediaId, usage }`.
 ...
 * any remaining text. The media sentinel (`[[SEND_MEDIA:<id>]]`) is
 * parsed the same way and its id extracted -- malformed/unclosed
 * markers are left as plain text rather than guessed at. `usage` is
...
export function parseGeneration(
  ...
  text = text.trim()
    return { text, handoff, mediaId, usage }
}
