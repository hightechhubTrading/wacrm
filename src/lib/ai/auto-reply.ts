...
export async function dispatchInboundToAiReply(
  ...
  try {
      ...
      // id (best-effort; empty when the account hasn't set one up, in
      // which case buildSystemPrompt omits the whole capability).
      ...
      const { text, handoff, mediaId, usage } = await generateReply({
              config,
              systemPrompt,
              messages,
      })
